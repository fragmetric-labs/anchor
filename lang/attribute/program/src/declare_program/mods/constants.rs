...
pub fn gen_constants_mod(idl: &Idl) -> proc_macro2::TokenStream {
    let constants = idl.constants.iter().map(|c| {
        ...
        let val = match &c.ty {
            IdlType::Pubkey => {
                let address_bytes = bs58::decode(&c.value)
                    .into_vec()
                    .unwrap();
                quote! {
                    anchor_lang::solana_program::pubkey::Pubkey::new_from_array([#(#address_bytes,)*])
                }
            },
            _ => syn::parse_str::<syn::Expr>(&c.value)
                .unwrap()
                .to_token_stream()
        };
        let (ty, val) = match &c.ty {
            ...
            IdlType::Pubkey => (quote!(anchor_lang::solana_program::pubkey::Pubkey), val),
